...
\]
Here, $\src{e}$ is a continuation that passes its value to the bound variable
$x$. We maintain the invariant that ``If $\Gamma\vdash e:\tau \rightarrow
x.\src{e}$, then $\src{\Gamma}x:\neg\src{\tau}\vdash\src{e}:0$.''

In respect of convention, we'll strive to use the variable $k$ instead of $x$ as
the continuation variable here. One hopes that this does not cause the reader
any great difficulty, as we also often use the variable $k$ for kinds.
...
We won't talk about sums, references, exns, primitives, or recursive types here.
These are left as an exercise for the reader.

\subsection{Exceptions}

We're basically just going to go through everything and redo it. However, the
rewrites are pretty straightforward - we're basically just going to pass a
failure continuation in through everything.
